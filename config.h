...
static char *fonts[] = {"Martian Mono Nerd Font:size=8"};
...
static const char *lsymbol =
    "<"; /* shown when there are more items on the left */
static const char *rsymbol =
    ">"; /* shown when there are more items on the right */
...
/* By default the powerline separator will take up the full space between dmenu
 * items. This option allows for the size to be reduced by a number of pixels,
 * e.g. a value of 3 will shave off three pixels on each side of the separator.
 * This can be used to adjust the angle of a powerline slash or arrow. */
...
static unsigned long functionality =
    ...
    //	|ContinuousOutput // makes dmenu print out selected items immediately
    // rather than at the end
    | FuzzyMatch // allows fuzzy-matching of items in dmenu
    //	|HighlightAdjacent // makes dmenu highlight items adjacent to the
    // selected item 	|Incremental // makes dmenu print out the current text
    // each time a key is pressed 	|InstantReturn // makes dmenu select an
    // item immediately if there is only one matching option left 	|Managed
    // // allow dmenu to be managed by window managers (disables
    // override_redirect) 	|NoInput // disables the input field in dmenu,
    // forcing
    // the user to select options using mouse or keyboard 	|PasswordInput
    // // indicates that the input is a password and should be masked
    // |PrintIndex // makes dmenu print out the 0-based index instead of the
    // matched text itself 	|PrintInputText // makes dmenu print the input
    // text
    // instead of the selected item
    | PromptIndent // makes dmenu indent items at the same level as the prompt
                   // on multi-line views
    //	|RejectNoMatch // makes dmenu reject input if it would result in no
    // matching item 	|RestrictReturn // disables Shift-Return and Ctrl-Return
    // to restrict dmenu to only output one item 	|ShowNumbers // makes
    // dmenu display the number of matched and total items in the top right
    // corner
...
/* Alpha values. You only need to add colour schemes here if you want different
 * levels of transparency per scheme. The default values are defined in the
 * alpha_default array in drw.c. */
